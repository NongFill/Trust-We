...
export default function Detail_Deposit() {
  ...
  const handleAcceptOrder = (order) => {
    ...
    if (!order.status || order.status === "รอดำเนินการ") {
      ...
      message = `ยืนยันว่าคำสั่งซื้อนี้ได้ฝากเรียบร้อยและเปลี่ยนสถานะเป็น 'รอคืนของกับลูกค้า'?`;
      confirmAction = async () => {
        try {
          const serveOrderRef = doc(db, 'serve', user.uid, 'orders', order.id);
          const userOrderRef = doc(db, 'users', order.userId, 'orders', order.id);
      ...
      message = `เมื่อยืนยัน สถานะจะเปลี่ยนเป็น 'ดำเนินการเสร็จสิ้น' และคำสั่งซื้อจะถูกย้ายไปยังประวัติทันที`;
      confirmAction = async () => {
        try {
          const serveOrderRef = doc(db, 'serve', user.uid, 'orders', order.id);
          const userOrderRef = doc(db, 'users', order.userId, 'orders', order.id);
          ...
          await updateDoc(userOrderRef, { status: 'ดำเนินการเสร็จสิ้น' });
